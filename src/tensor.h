...
class Tensor {
 public:
   ...
   * @param dimensions std::vector<std::size_t> with the ordered dimensions of
   * the indices.
   * @param data pointer to the data of the tensor. If provided, it is
   * responsibility of the user to provide enough allocated memory to store the
   * Tensor and to deallocate it after its use.
   */
  Tensor(std::vector<std::string> indices, std::vector<std::size_t> dimensions,
         s_type* data = nullptr);
