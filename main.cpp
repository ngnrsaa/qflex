...
int main(int argc, char **argv) {
  ...
  talsh::initialize(&size);
  {
    ...
    */
    /*
    unsigned int remains = conts_gpu.size();
    auto its = conts_gpu.begin();
    ...
    while (remains > 0)
    {
      while (its != conts_gpu.end())
      {
        errc = its->execute(DEV_NVIDIA_GPU,0); // Tries to schedule execution
      ...
      for (auto itc=conts_gpu.begin(); itc!=its; ++itc) // Check scheduled cs
      {
        int sts;
