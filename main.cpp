...
int main(int argc, char **argv) {
  ...
  */
  ...
  for (int cut=0; cut<cut_combinations_taken.size(); ++cut)
  {
    ...
    // ADDED TALSH
    H_5_legs_c.set_indices_and_dimensions({"(7,8),(7,9)","(6,8),(6,9)",
                            "(5,8),(5,9)", "(3,8),(3,9)","(4,8),(4,9)"},
                            vector<int>(5,super_dim));
    talsh::Tensor Rsh(H_7_legs_a.get_dimensions(), H_7_legs_a.data());
    talsh::Tensor Ssh(H_6_legs_a.get_dimensions(), H_6_legs_a.data());
    ...
    t1 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t1 - t0);
    cout << "\nTime spent contracting AB: " << time_span.count() << "s\n\n";
    time_in_loops += time_span.count();
